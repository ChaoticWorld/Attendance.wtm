...
namespace Attendance
{
    public partial class AttendanceBLL
    {
        ...
        public AttendanceBLL(int userID) {
        ...
        public int DoAtte(int eid, DateTime begin, DateTime end) {
            ...
            try {
                //dingTalkAttendance
                //获取指定eid员工的DingTalkAttendance记录，转换成List<ICardTime>
                DingTalkAttendance dingTalkAttendance = new DingTalkAttendance();
                List<ICardTime> dingTalkCardTimes = new List<ICardTime>();
                var dingTalkAttes = dingTalkAttendance.getUserCheckTime(getEmpName(eid), begin);
                if (dingTalkAttes != null)
                    foreach (var rrd in dingTalkAttes) {
                        HWATTDB.CardTime cardTime = new HWATTDB.CardTime() { EmployeeId = eid, cardTime = dingTalkAttendance.GetDateTime(rrd.UserCheckTime) };
                        dingTalkCardTimes.Add(cardTime);
                    }

                //dingTalkAttendance.getUserCheckTime
                //if (cardTimeDays == null) //cardTimeDays = 
                //else cardTimeDays.AddRange();
                ...
                DateTime b = begin;
                r = (int)Math.Floor(end.Subtract(begin).TotalDays);
                //HWATT
                while (b <= end) {
                    ...
                    if (cardTimes.Exists(e => e.cardTime.Date == b.Date)) {
                        ...
                        if (dingTalkCardTimes.Exists(e => e.cardTime.Date == b.Date))
                            cardTimeDays.AddRange(dingTalkCardTimes.FindAll(f => f.cardTime.Date == b.Date));
                    } else {
                        if (dingTalkCardTimes.Exists(e => e.cardTime.Date == b.Date))
                            cardTimeDays = dingTalkCardTimes.FindAll(f => f.cardTime.Date == b.Date);
                        else
                            cardTimeDays = null;
                    }
        ...
        public int autoFillAttr(int eid) {
            ...
            else begin = device.cardBll.getStartDate(0);//考勤系统最早记录时间
            end = DateTime.Now;
        ...
        public string getEmpName(int emdId) {
            string empName = string.Empty;
            EmployeeRefSchemeClassDAL erscDal = new EmployeeRefSchemeClassDAL();
            var emps = erscDal.selects(new EmployeeRefSchemeClassModel() { EmployeeID = emdId });
            if (emps != null && emps.Count > 0)
                empName = emps.OrderByDescending(o => o.isVaild).First().EmployeeName;
            else { throw new InvalidOperationException(emdId + " - 无此员工！"); }
            return empName;
        }
